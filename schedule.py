          ...
          if ' schedule function ' in line and ' as @s ' in line:
            ...
            with open(f'{os.path.join(schedule_func_path, func_uuid)}.mcfunction', 'w') as file:
              file.writelines([
                ...
                f'schedule function {schedule_func_path.split('\\')[3]}:_schedule/{schedule_uuid} {ticks}{replace_append}'
              ])
            ...
            new_command[-1] = f'{schedule_func_path.split('\\')[3]}:_schedule/{func_uuid}'
            ...
            if (name := line.split()[-1]) in [d['name'] for d in scheduled_names]:
              lines[i] = line.replace(name, f'{schedule_func_path.split('\\')[3]}:_schedule/{(this_scheduled_name := ([dict(uuid = d['uuid'], tag = d['tag']) for d in scheduled_names if d['name'] == name][0]))['uuid']}')
